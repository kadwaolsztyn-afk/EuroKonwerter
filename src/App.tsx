...
export default function App() {
  ...
  const handlePushToGitHub = async () => {
    ...
    const persistentPat = localStorage.getItem('carlamps_github_pat_persistent') || '';
    const tokenToUse = (config.githubToken || persistentPat).trim();
    if (!tokenToUse) {
      setMainTab('settings');
      showNotification(
        'Wprowadź token GitHub (Personal Access Token), aby móc automatycznie publikować na GitHub i Vercel.'
      );
      return;
    }
    ...
    try {
      setIsPushingGH(true);
      const res = await pushDatabaseToGitHub(currentDocument, tokenToUse);
      if (res.success) {
        showNotification(
