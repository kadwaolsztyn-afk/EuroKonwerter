...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  // State for Full Code & Database Push to GitHub
  const [isPushingFullCode, setIsPushingFullCode] = useState(false);
  const [isPushingDatabase, setIsPushingDatabase] = useState(false);
  ...
  const handleOpenPushFullCode = () => {
    ...
    const savedToken = ghConfig.githubToken || localStorage.getItem('carlamps_github_pat_persistent') || '';
    setPatTokenInput(savedToken);
    setShowFullCodeModal(true);
  };

  const handleExecutePushDatabase = async () => {
    const tokenToUse = (patTokenInput.trim() || ghConfig.githubToken?.trim() || localStorage.getItem('carlamps_github_pat_persistent') || '').trim();
    if (!tokenToUse) {
      setPushResultError('Wprowadź token GitHub (Personal Access Token), aby móc wysłać zaktualizowaną bazę.');
      return;
    }

    try {
      setIsPushingDatabase(true);
      setPushResultError(null);
      setPushResultMessage('Zapisywanie bazy, optymalizacja zdjęć i wysyłanie do GitHub (Auto-Deploy Vercel)...');

      try {
        localStorage.setItem('carlamps_github_pat_persistent', tokenToUse);
        await saveGitHubSyncConfig({ githubToken: tokenToUse });
      } catch (_) {}

      const res = await pushDatabaseToGitHub(document, tokenToUse);
      if (res.success) {
        setPushResultMessage(
          res.message || 'Pomyślnie wysłano zaktualizowaną bazę ze zdjęciami do GitHub! Vercel natychmiast publikuje aktualizację.'
        );
        if (res.commitSha) {
        ...
        if (onNotification) {
          onNotification('Pomyślnie wysłano zaktualizowaną bazę ze zdjęciami do GitHub! Vercel publikuje nową wersję.');
        }
      } else {
        setPushResultError(res.error || 'Wystąpił błąd podczas wysyłania bazy do GitHub.');
      }
    } catch (err: any) {
      setPushResultError(err?.message || 'Nieznany błąd podczas wysyłania bazy.');
    } finally {
      setIsPushingDatabase(false);
    }
  };
  ...
  return (
    <div className="w-full">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <button
                  ...
                  onClick={handleOpenPushFullCode}
                  disabled={isPushingToGitHub || isPushingDatabase || isPushingFullCode}
                  ...
                  title="Wyślij zaktualizowaną bazę ze zdjęciami lub cały kod programu do GitHub i uruchom auto-deploy na Vercel"
                >
                  <GitCommit className={`w-4 h-4 ${(isPushingToGitHub || isPushingDatabase) ? 'animate-spin' : ''}`} />
                  <span>
                    {isPushingToGitHub || isPushingDatabase
                      ? 'Wysyłanie do GitHub...'
              ...
              <div className="flex items-center gap-1.5 flex-wrap justify-end w-full sm:w-auto">
                ...
                {onPushToGitHub && (
                  <button
                    ...
                    title="Wyślij zaktualizowaną bazę ze zdjęciami lub cały kod programu do GitHub i uruchom auto-deploy na Vercel"
                  >
                    <GitCommit className={`w-3.5 h-3.5 ${(isPushingToGitHub || isPushingDatabase) ? 'animate-spin' : ''}`} />
                    <span className="hidden sm:inline">
                      {isPushingToGitHub || isPushingDatabase ? 'Wysyłanie...' : 'Wyślij do GitHub / Vercel'}
                    </span>
                  </button>
                )}
      ...
      {/* Modal: Publikacja w GitHub i Vercel (Baza / Pełny kod) */}
      {showFullCodeModal && (
          ...
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-xl w-full p-6 shadow-2xl relative text-left my-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white flex items-center gap-2">
                    <span>Publikacja na GitHub & Auto-Deploy Vercel</span>
                  </h3>
                  <p className="text-xs text-slate-400">
                    Repozytorium: <span className="text-amber-400 font-mono">kadwaolsztyn-afk/EuroKonwerter</span> (gałąź <span className="text-white font-mono">main</span>)
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="py-4 space-y-4 text-xs sm:text-sm text-slate-300">
              ...
              <div className="grid grid-cols-1 gap-2.5">
                <div className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-3.5 space-y-1.5">
                  <div className="font-semibold text-emerald-300 flex items-center gap-2 text-xs">
                    <Database className="w-4 h-4 text-emerald-400 shrink-0" />
                    <span>Opcja 1: Wyślij zaktualizowaną bazę ze zdjęciami</span>
                  </div>
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Wysyła najnowszą bazę cennika (<code className="text-slate-200">data-catalog.json</code>, <code className="text-slate-200">initialCatalog.ts</code>) oraz wszystkie zdjęcia lamp z folderu <code className="text-slate-200">public/uploads/</code>. Vercel natychmiast publikuje zaktualizowany cennik online.
                  </p>
                </div>

                <div className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-3.5 space-y-1.5">
                  <div className="font-semibold text-purple-300 flex items-center gap-2 text-xs">
                    <Rocket className="w-4 h-4 text-purple-400 shrink-0" />
                    <span>Opcja 2: Wypchnij CAŁY kod programu do GitHub</span>
                  </div>
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Wysyła wszystkie pliki źródłowe (komponenty React, style Tailwind, pliki serwera, konfigurację Vite i bazę). Powoduje pełny przebudowanie programu na Vercelu.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-slate-500">
                  Wymaga uprawnienia <strong className="text-slate-400">repo</strong>. Token zostanie bezpiecznie zapamiętany w Twojej przeglądarce i nie musisz go wpisywać ponownie.
                </p>
              </div>
              ...
              {pushResultError && (
                <div className="p-3.5 rounded-xl bg-red-950/60 border border-red-500/40 text-red-300 text-xs flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <div className="whitespace-pre-wrap">{pushResultError}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="pt-4 border-t border-slate-800 flex flex-wrap items-center justify-end gap-2.5">
              <button
                type="button"
                onClick={() => setShowFullCodeModal(false)}
                disabled={isPushingFullCode || isPushingDatabase}
              ...
              <button
                type="button"
                onClick={handleExecutePushDatabase}
                disabled={isPushingDatabase || isPushingFullCode}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold shadow-md shadow-emerald-600/25 border border-emerald-400/40 transition-all cursor-pointer disabled:opacity-50"
              >
                <Database className={`w-3.5 h-3.5 ${isPushingDatabase ? 'animate-spin' : ''}`} />
                <span>
                  {isPushingDatabase
                    ? 'Wysyłanie bazy...'
                    : 'Wyślij bazę ze zdjęciami'}
                </span>
              </button>
              ...
              <button
                ...
                disabled={isPushingDatabase || isPushingFullCode}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white text-xs font-bold shadow-md shadow-purple-600/30 border border-purple-400/50 transition-all cursor-pointer disabled:opacity-50"
              >
                <Rocket className={`w-3.5 h-3.5 text-amber-300 ${isPushingFullCode ? 'animate-bounce' : ''}`} />
                <span>
                  {isPushingFullCode
                    ? 'Wysyłanie całego kodu...'
                    : 'Wypchnij CAŁY kod programu'}
                </span>
              </button>
            </div>
          </div>
