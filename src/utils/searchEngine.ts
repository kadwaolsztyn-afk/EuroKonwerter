...
const GENERIC_MODEL_PREFIXES = new Set(['seria', 'klasa', 'model']);

/**
 * Checks if a row model matches the selected model filter.
 * Handles exact matches, case/diacritic normalization, compact representation,
 * and token-based matching across punctuation differences (e.g. 'A4, S4' vs 'A4-S4').
 */
export function matchesModelFilter(rowModelStr?: string, targetModelStr?: string): boolean {
  if (!targetModelStr || targetModelStr === 'all') return true;
  if (!rowModelStr) return false;

  const rowModel = normalizeForSearch(rowModelStr);
  const targetModel = normalizeForSearch(targetModelStr);

  if (rowModel === targetModel) return true;

  // 1. Direct or clean substring matching
  const cleanTarget = targetModel.replace(/\([^)]+\)/g, '').trim();
  if (rowModel.includes(targetModel) || (cleanTarget.length >= 2 && rowModel.includes(cleanTarget)) || targetModel.includes(rowModel)) {
    return true;
  }

  // 2. Compact matching (ignoring hyphens, commas, slashes, spaces)
  const compactRow = toCompactSearch(rowModelStr);
  const compactTarget = toCompactSearch(targetModelStr);
  if (compactRow && compactTarget) {
    if (compactRow === compactTarget) return true;
    if (compactTarget.length >= 4 && compactRow.includes(compactTarget)) return true;
    if (compactRow.length >= 4 && compactTarget.includes(compactRow)) return true;
  }

  // 3. Token-based matching (preserving critical model digits/letters like '3', '4', 'x3', 'a4')
  const targetTokens = cleanTarget
    .split(/[\s,\-\/]+/)
    .map((t) => t.trim())
    .filter((t) => t.length > 0 && !GENERIC_MODEL_PREFIXES.has(t));
  const rowTokens = rowModel
    .split(/[\s,\-\/]+/)
    .map((t) => t.trim())
    .filter((t) => t.length > 0 && !GENERIC_MODEL_PREFIXES.has(t));

  if (targetTokens.length > 0 && targetTokens.every((tt) => rowTokens.some((rt) => rt === tt))) {
    return true;
  }
...
export function filterCatalogRows(
  ...
  const filteredRows = rows.filter((row) => {
    ...
    // 2. Model
    if (effectiveModel && effectiveModel !== 'all') {
      if (!matchesModelFilter(row.model, effectiveModel)) {
        return false;
      }
    }
