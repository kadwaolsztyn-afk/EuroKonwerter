...
export const PERSISTENT_PAT_KEY = 'carlamps_github_pat_persistent';
...
export function getGitHubSyncConfig(): GitHubSyncConfig {
  try {
    ...
    const persistentPat = localStorage.getItem(PERSISTENT_PAT_KEY);
    ...
    if (saved) {
      ...
      // If token not in config, check persistent storage
      if (!parsed.githubToken && persistentPat) {
        parsed.githubToken = persistentPat;
      }
      ...
      try {
        ...
        const initialCfg = {
          ...DEFAULT_CONFIG,
          githubToken: persistentPat || DEFAULT_CONFIG.githubToken,
        };
        localStorage.setItem(CONFIG_STORAGE_KEY, JSON.stringify(initialCfg));
      } catch (_) {}
    }
  } catch (e) {
...
export async function saveGitHubSyncConfig(config: Partial<GitHubSyncConfig>): Promise<GitHubSyncConfig> {
  ...
  try {
    ...
    if (config.githubToken && typeof config.githubToken === 'string' && config.githubToken.trim()) {
      localStorage.setItem(PERSISTENT_PAT_KEY, config.githubToken.trim());
    }
  } catch (e) {
...
export async function pushDatabaseToGitHub(
  ...
  const config = getGitHubSyncConfig();
  const persistentPat = localStorage.getItem(PERSISTENT_PAT_KEY) || '';
  const authToken = (token || config.githubToken || persistentPat || '').trim();
  if (!authToken) {
    return {
      success: false,
  ...
  try {
    localStorage.setItem(PERSISTENT_PAT_KEY, authToken);
  } catch (_) {}
  ...
  try {
    ...
    if (serverRes.ok) {
      ...
      if (data && data.success) {
        ...
        console.warn('[GitHub Sync] Server push returned error, falling back to direct Git Data API:', data.error);
      }
    } else {
      const errData = await serverRes.json().catch(() => ({}));
      console.warn('[GitHub Sync] Server push returned HTTP error, falling back to direct Git Data API:', errData?.error || serverRes.statusText);
    }
  } catch (serverErr) {
