...
export function sanitizeDocumentRows(rows: DocumentRow[]): DocumentRow[] {
  ...
  return rows.map((r, idx) => {
    ...
    // Repair known model name typos and inconsistencies
    if (brandTrim === 'BMW') {
      if (modelTrim === 'Sesia X3') {
        modelTrim = 'Seria X3';
      } else if (modelTrim === 'Seria 7 LCI') {
        modelTrim = 'Seria 7';
      }
    } else if (brandTrim === 'Jeep') {
      if (modelTrim === 'Grand Cherockee') {
        modelTrim = 'Grand Cherokee';
      }
      // Fix shifted columns in Jeep Compass row 264 where raw row had empty brand cell
      if (r.lp === '264' || (modelTrim === 'Compass' && r.years === 'Żarówka')) {
        return {
          ...r,
          id: r.id || 264,
          lp: '264',
          ...
          priceClientStatic: r.priceClientStatic && r.priceClientStatic !== '-' ? r.priceClientStatic : '700 zł',
          basePriceClientStatic: '490  zł',
          priceBrokerStatic: r.priceBrokerStatic && r.priceBrokerStatic !== '-' ? r.priceBrokerStatic : '343 zł',
    ...
    } else if (brandTrim === 'Dodge') {
      // Excel drag-down autofill typo where 5500 incremented to 5501 and 5502
      if (modelTrim === 'RAM 1500/2500/3500/5501' || modelTrim === 'RAM 1500/2500/3500/5502') {
        modelTrim = 'RAM 1500/2500/3500/5500';
      }
    } else if (brandTrim === 'Porsche') {
      if (modelTrim === 'Tycan') {
        modelTrim = 'Taycan';
      } else if (modelTrim.includes('Boxter')) {
        modelTrim = modelTrim.replace(/Boxter/g, 'Boxster');
      }
    } else if (brandTrim === 'Toyota' && modelTrim === 'Land Crusier') {
      modelTrim = 'Land Cruiser';
    } else if (brandTrim === 'Audi') {
      if (modelTrim.includes('Sportbact')) {
        modelTrim = modelTrim.replace(/Sportbact/g, 'Sportback');
      }
      if (modelTrim.includes('RSQ9')) {
        modelTrim = modelTrim.replace(/RSQ9/g, 'RSQ8');
      }
      if (modelTrim === 'Q5-SQ5 (Sportback)') {
        modelTrim = 'Q5, SQ5 (Sportback)';
      }
    }
