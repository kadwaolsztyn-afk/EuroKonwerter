...
export async function importDocumentFromFile(
  ...
  // 3a. Check for JSON format (backup files, data-catalog.json, exported catalog)
  ...
  if (
    ...
    try {
      ...
      let version = `2026.03_ALL_v461_IMPORT_${Date.now()}`;
      let headers: string[] = ['Lp.', 'Marka', 'Model', 'Generacja / Kod', 'Roczniki', 'Cena Stat. Klient', 'Cena Dyn. Klient'];
      ...
      if (parsed && Array.isArray(parsed.rows)) {
        ...
        rawRows = parsed.document.rows;
        if (parsed.document.name) docName = parsed.document.name;
        if (parsed.document.version) version = parsed.document.version;
        if (Array.isArray(parsed.document.headers)) headers = parsed.document.headers;
      } else if (Array.isArray(parsed)) {
        rawRows = parsed;
      }

      if (rawRows.length > 0) {
        const rows: DocumentRow[] = rawRows.map((r, idx) => {
          const id = typeof r.id === 'number' ? r.id : idx + 1;
          const brand = String(r.brand || r.Marka || '').trim();
          const model = String(r.model || r.Model || '').trim();
          const factoryCode = String(r.factoryCode || r['Generacja / Kod'] || r.generation || '-').trim();
          const years = String(r.years || r.Roczniki || r.roczniki || '').trim();
          const staticSignal = String(r.staticSignal || r['Kierunkowskaz Statyczny'] || '').trim();
          const priceClientStatic = String(r.priceClientStatic ?? (r.priceStatic !== undefined ? `${r.priceStatic} zł` : '')).trim();
          const priceBrokerStatic = String(r.priceBrokerStatic ?? (r.wholesalePriceStatic !== undefined ? `${r.wholesalePriceStatic} zł` : '')).trim();
          const dynamicSignal = String(r.dynamicSignal || r['Kierunkowskaz Dynamiczny'] || '').trim();
          const priceClientDynamic = String(r.priceClientDynamic ?? (r.priceDynamic !== undefined ? `${r.priceDynamic} zł` : '')).trim();
          const priceBrokerDynamic = String(r.priceBrokerDynamic ?? (r.wholesalePriceDynamic !== undefined ? `${r.wholesalePriceDynamic} zł` : '')).trim();
          const installation = String(r.installation || r['Montaż'] || '').trim();
          const coding = String(r.coding || r['Kodowanie'] || '').trim();
          const lampCount = String(r.lampCount || r['Liczba lamp'] || '').trim();
          const imageUrl = r.imageUrl || r.zdjecie || '';
          const imageAlt = r.imageAlt || `${brand} ${model}`;
          const customNotes = r.customNotes || r.notes || r.uwagi || '';
          ...
          return {
            ...
            staticSignal,
            priceClientStatic,
            basePriceClientStatic: r.basePriceClientStatic || priceClientStatic,
            priceBrokerStatic,
            basePriceBrokerStatic: r.basePriceBrokerStatic || priceBrokerStatic,
            dynamicSignal,
            priceClientDynamic,
            basePriceClientDynamic: r.basePriceClientDynamic || priceClientDynamic,
            priceBrokerDynamic,
            basePriceBrokerDynamic: r.basePriceBrokerDynamic || priceBrokerDynamic,
            installation,
            coding,
            lampCount,
            imageUrl,
            imageAlt,
            customNotes,
            multimediaVersion: r.multimediaVersion,
            multimediaPriceClient: r.multimediaPriceClient,
            multimediaPriceBroker: r.multimediaPriceBroker,
            multimediaImageUrl: r.multimediaImageUrl,
            multimediaNotes: r.multimediaNotes,
            multimediaItems: Array.isArray(r.multimediaItems) ? r.multimediaItems : undefined,
            rawCells: Array.isArray(r.rawCells) ? r.rawCells : undefined,
          };
        });

        const brandsCount = new Set(rows.map((r) => (r.brand || '').trim()).filter((b) => b && b !== '-')).size;
      ...
      console.warn('JSON parse attempt failed, falling through to text/SheetJS parser:', jsonErr);
    }
  }
