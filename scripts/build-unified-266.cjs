...
const build260 = fs.readFileSync(path.join(__dirname, 'build-260-models.cjs'), 'utf8');
const extraMatch = build260.match(/const EXTRA_MODELS = \[([\s\S]*?)\];\s*console\.log/);
let EXTRA_MODELS = [];
if (extraMatch) {
  eval('EXTRA_MODELS = [' + extraMatch[1] + '];');
}

const additionalModels = [
  { brand: 'Audi', model: 'Q8 / SQ8 / RS Q8 (4M8)', factoryCode: '4M8', years: '2018-2024', pClientStat: '2 200 zł', pBrokStat: '1 760 zł', pClientDyn: '3 400 zł', pBrokDyn: '2 720 zł', statSig: 'Statyczny LED', dynSig: 'HD Matrix LED Laser Animacja', lamps: '4 szt. + listwa', cod: 'ODIS Online w cenie' },
  { brand: 'BMW', model: 'X5 (G05 LCI / M60i)', factoryCode: 'G05 LCI', years: '2023-2024', pClientStat: '2 100 zł', pBrokStat: '1 680 zł', pClientDyn: '3 200 zł', pBrokDyn: '2 560 zł', statSig: 'Statyczny LED', dynSig: 'Matrix X-Shape Dynamic LED', lamps: '4 szt.', cod: 'ISTA Online w cenie' },
  { brand: 'Mercedes-Benz', model: 'Klasa G (W463A / G63 AMG)', factoryCode: 'W463A', years: '2018-2024', pClientStat: '2 400 zł', pBrokStat: '1 920 zł', pClientDyn: '3 600 zł', pBrokDyn: '2 880 zł', statSig: 'Statyczny LED', dynSig: 'Multibeam 3D Dynamic LED', lamps: '2 szt. (okrągłe)', cod: 'DTS Monaco w cenie' },
  { brand: 'Porsche', model: 'Taycan / Taycan Cross Turismo (J1)', factoryCode: 'Y1A', years: '2020-2024', pClientStat: '2 400 zł', pBrokStat: '1 920 zł', pClientDyn: '3 600 zł', pBrokDyn: '2 880 zł', statSig: 'Statyczny LED', dynSig: '4-Point Matrix LED + Listwa 3D', lamps: '2 szt. + pas świetlny', cod: 'PIWIS 3 Online w cenie' },
  { brand: 'Ford', model: 'Mustang Mach-E (CX727)', factoryCode: 'CX727', years: '2020-2024', pClientStat: '1 600 zł', pBrokStat: '1 280 zł', pClientDyn: '2 450 zł', pBrokDyn: '1 960 zł', statSig: 'Statyczny LED', dynSig: '3-Paskowy Sekwencyjny LED', lamps: '2 szt. (tribal)', cod: 'FDRS Online w cenie' },
  { brand: 'Dodge', model: 'RAM 1500 (DT / TRX / Limited)', factoryCode: 'DT 5gen', years: '2019-2024', pClientStat: '1 500 zł', pBrokStat: '1 200 zł', pClientDyn: '2 250 zł', pBrokDyn: '1 800 zł', statSig: 'Statyczny LED ECE', dynSig: 'Bi-LED Projektor Dynamic', lamps: '2 szt.', cod: 'AlfaOBD / WiTECH w cenie' },
  { brand: 'Chevrolet', model: 'Tahoe / Suburban (GMT T1XX)', factoryCode: 'T1XX', years: '2021-2024', pClientStat: '1 650 zł', pBrokStat: '1 320 zł', pClientDyn: '2 550 zł', pBrokDyn: '2 040 zł', statSig: 'Statyczny LED ECE', dynSig: 'C-Shape Dynamic LED', lamps: '2 szt.', cod: 'GDS2 Online w cenie' },
  { brand: 'Cadillac', model: 'Escalade V (T1XX / 600)', factoryCode: 'T1XX', years: '2021-2024', pClientStat: '2 200 zł', pBrokStat: '1 760 zł', pClientDyn: '3 400 zł', pBrokDyn: '2 720 zł', statSig: 'Statyczny LED', dynSig: 'Wertykalne Ostrza 3D Matrix', lamps: '2 szt. (wertykalne 1m)', cod: 'GDS2 Online w cenie' }
];

const ALL_MODELS = [...MODELS_DATA, ...EXTRA_MODELS, ...additionalModels];
...
const uniqueBrands = Array.from(new Set(ALL_MODELS.map(m => m.brand))).sort();
...
// 2. Write public/data-catalog.json
if (!fs.existsSync(path.join(process.cwd(), 'public'))) {
  fs.mkdirSync(path.join(process.cwd(), 'public'));
}
fs.writeFileSync(path.join(process.cwd(), 'public', 'data-catalog.json'), JSON.stringify(comprehensiveDocument, null, 2), 'utf8');

// 3. Write src/data/initialCatalog.ts
const docForTs = { ...comprehensiveDocument };
delete docForTs.importedAt;

const tsContent = `import { ImportedDocument } from '../types';
...
`;
fs.writeFileSync(path.join(process.cwd(), 'src/data/initialCatalog.ts'), tsContent, 'utf8');

// 4. Write src/data/sampleDocument.ts
fs.writeFileSync(
  path.join(process.cwd(), 'src/data/sampleDocument.ts'),
  `import { ImportedDocument } from '../types';\nimport { INITIAL_COMPREHENSIVE_CATALOG } from './initialCatalog';\n\nexport const SAMPLE_DOCUMENT: ImportedDocument = INITIAL_COMPREHENSIVE_CATALOG;\n`,
  'utf8'
);

console.log('SUCCESS: Generated master 266-model database across 35 brands!');
